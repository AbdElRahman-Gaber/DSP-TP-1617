...
sq = square(2*pi*f*t);
Nsq = length(sq);
fsq = (-Nsq/2 : Nsq/2-1)*fs/Nsq;
xfsq = fftshift(fft(sq));

figure;
subplot(221); plot(t, sq); title('Signal'); xlabel('Time(sec)'); ylabel('Amplitude');
subplot(222); plot(fsq, abs(xfsq)); title('Magnitude'); xlabel('Frequency'); ylabel('|X(f)|');
subplot(223); plot(fsq, real(xfsq)); title('Real'); xlabel('Frequency'); ylabel('Re(X(f))');
subplot(224); plot(fsq, imag(xfsq)); title('Imaginary'); xlabel('Frequency'); ylabel('Im(X(f))');
